...
bool process_record_kb(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case GLOBE_FN:
            if (record->event.pressed) {
                // Globe key
                // Per https://developer.apple.com/accessories/Accessory-Design-Guidelines.pdf
        ...
        case MAC_LOCK:
            if (record->event.pressed) {
                // Power / Lock signal
                // Per https://www.usb.org/sites/default/files/hut1_2.pdf ("AL Terminal Lock / Screensaver")
